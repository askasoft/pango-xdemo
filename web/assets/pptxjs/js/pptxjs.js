 ...
 * pptxjs.js
 * Ver. : 1.21.1
 * last update: 16/11/2021 (modified by oo)
...
(function ($) {
    $.fn.pptxToHtml = function (options) {
        ...
        renderSync(settings.data);

        function renderSync(data) {
            var zip = new JSZip().load(data);
        ...
        function processPPTX(zip) {
            ...
            var thumbnail = zip.file("docProps/thumbnail.jpeg");
            if (thumbnail) {
                post_ary.push({
                    "type": "pptx-thumb",
                    "data": base64ArrayBuffer(thumbnail.asArrayBuffer()),
                    "slide_num": -1
                });
            }
        ...
        function readXmlFile(zip, filename, isSlideContent) {
            try {
                var zf = zip.file(filename);
                if (!zf) {
                    return null;
                }

                var fileContent = zf.asText();
        ...
        function processSingleSlide(zip, sldFileName, index, slideSize) {
            ...
            */
            ...
            var result = "<div class='slide' style='width:" + slideSize.width + "px; height:" + slideSize.height + "px;" + bgColor + "'>" + bgResult;
        ...
        */
        function processPicNode(node, warpObj, source, sType) {
            ...
            if (!imgFile) {
                console.error('missing file: ', imgName);
                return "";
            }
