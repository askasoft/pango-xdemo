	...
	if (!$pc.length) {
		$pc = $('<div id="s_popup_confirm" class="ui-popup s-popup-confirm" popup-mask="true" popup-position="center" popup-closer="false">'
				...
				+ '<div class="ui-popup-body">'
					+ '<i class="icon fa fa-3x fa-question-circle"></i>'
					+ '<div class="msg"></div>'
				+ '</div>'
				+ '<div class="ui-popup-footer">'
					+ '<button class="btn btn-primary ok"><i class="fa fa-check"></i> <span>OK</span></button>\n'
					+ '<button class="btn btn-default cancel" popup-dismiss="true"><i class="fa fa-times"></i> <span>Cancel</span></button>'
				+ '</div>'
			+ '</div>'
		);
		$pc.popup();
